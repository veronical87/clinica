...
public class GestionHorarios extends javax.swing.JDialog {
    Connection cn;
    ConexionMySQL cm=new ConexionMySQL();
    CallableStatement cmd;
    ClaseHorarios hora=new ClaseHorarios();
    DefaultTableModel modelo;
    public GestionHorarios(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        setLocationRelativeTo(null);
        modelo = (DefaultTableModel) TABLA.getModel();
        hora.LlenarTabla(modelo);
    }
    ...
    private void initComponents() {
        ...
        jPanel3.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(0, 0, 0), 2), "LISTA DE HORARIOS", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Tahoma", 1, 12))); // NOI18N
        ...
        jComboBox_CriterioSeleccionado.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "Hora", "Todos" }));
        ...
        PANEL_BUSQUEDALayout.setHorizontalGroup(
            ...
            .addGroup(PANEL_BUSQUEDALayout.createSequentialGroup()
                ...
                .addGap(33, 33, 33)
                .addComponent(jTextField_criteriodeBusqueda, javax.swing.GroupLayout.PREFERRED_SIZE, 175, javax.swing.GroupLayout.PREFERRED_SIZE)
...
int filasel,IDROL,IDUSUARIO;
    ...
    private void buttonTaskAGREGARActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_buttonTaskAGREGARActionPerformed
        dispose();
        AltaHorario au = new AltaHorario(new javax.swing.JFrame(), true);
        au.idrol = IDROL;        
        au.show();
    ...
    private void buttonTaskEDITARActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_buttonTaskEDITARActionPerformed
        if (filasel != -1) {            
            ModificarHorario mu = new ModificarHorario(new javax.swing.JFrame(), true);            
            mu.idrol = IDROL;
            mu.id=idHora;
            mu.jSpinnerHORA.getModel().setValue(Hora);
            mu.jSpinnerMINUTOS.getModel().setValue(Minutos);
            dispose();
            mu.show();
        } else {
    ...
    private void buttonTaskBORRARActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_buttonTaskBORRARActionPerformed
        if(filasel!=-1){
            int resultado=hora.EliminarDatosHora(idHora);            
            if(resultado !=0){
                JOptionPane.showMessageDialog(null,"El Horario Solicitado esta Siendo Utilizado, No Puede ser Borrado","Información",JOptionPane.INFORMATION_MESSAGE);
                this.buttonTaskAGREGAR.setEnabled(true);
                this.buttonTaskEDITAR.setEnabled(false);
                this.buttonTaskBORRAR.setEnabled(false);
            }else{
                if(JOptionPane.showConfirmDialog(null,"¿Desea Eliminar el Horario"+" "+Hora+":"+Minutos+"?","Consulta",JOptionPane.OK_CANCEL_OPTION) == JOptionPane.OK_OPTION) {
                    JOptionPane.showMessageDialog(null,"El Horario"+" "+Hora+":"+Minutos+" Se Borró Correctamente","Información",JOptionPane.INFORMATION_MESSAGE);
                    limpiarTabla(TABLA);
                    modelo = (DefaultTableModel) TABLA.getModel();
                    hora.LlenarTabla(modelo);
                    this.buttonTaskAGREGAR.setEnabled(true);
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
               GestionHorarios dialog = new GestionHorarios(new javax.swing.JFrame(), true);
...
int idHora,Hora;String Minutos;
private void BuscarDatosHora(String id) {
      cn=cm.Conectar();            
        String sSQL = "SELECT id,hora,minutos FROM horarios WHERE id="+id;
        try
           {
...
private void BuscarDatosxHora(String captura) {
    String[] titulos = {"ID","Hora"};
    modelo = new DefaultTableModel(null,titulos);
    cn=cm.Conectar();
    String sSQL = "";
    String[] registro = new String[2];
    sSQL = "SELECT id,hora,minutos FROM horarios WHERE hora='"+captura+"' OR minutos LIKE '"+captura+"%'";
    try
       {
        Statement st = (Statement) cn.createStatement();
        ResultSet rs = st.executeQuery(sSQL);
